...
router.post('/search', async (ctx, next) => {
	...
	for (var i = 0; i < result3.length; i++) {
		...
		let ssry = await query(
			`SELECT * FROM ssxs WHERE state = 1 && sushe = ${result3[i].sushe} && lynumber = '${lynumber}'`);
		// 格式化 ssry 的数据类型
...
router.post('/adduser', async (ctx, next) => {
	...
	var stuNumber = ctx.request.body.stuNumber;
	...
	let insertMysql = await query(
		`insert into ssxs(stuNumber,name,sex,xueyuan,banji,tel,sushe,lynumber,fdy,fdytel,state) values('${stuNumber}','${pass}','${name}','${sex}','${xueyuan}','${banji}','${tel}','${sushe}','${lynumber}','${fdy}','${fdytel}',1)`
	);
...
router.post('/del', async (ctx, next) => {
	...
	let a = await query(`update ssxs set state = 0 where id = ${id}`);
